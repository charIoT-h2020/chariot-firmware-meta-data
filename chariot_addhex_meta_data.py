...
try:
    ...
    with open(args.hex_name,'rb') as hex_file, open(output_file,'wb') as hexm_file:
        ...
        if args.all is not None:
            ...
            hexm_file.write(":chariot_md:")
            add_size += len(":chariot_md:")
            if args.verbose:
                print ("generate sha256 of " + args.hex_name)
            hexm_file.write(":sha256:")
            add_size += len(":sha256:")
            hexm_file.write(
                compute_sha_256_content(args.hex_name, verbose)
                    .to_bytes(256, byteorder='little', signed=False))
            add_size += 256/8
            if args.verbose:
            ...
            hexm_file.write(
                (len("!CHARIOTMETAFORMAT_2019a"))
                    .to_bytes(32, byteorder='little', signed=False))
            add_size += 32/8
            hexm_file.write("!CHARIOTMETAFORMAT_2019a")
            ...
            if additional_data_file is not None:
                ...
                hexm_file.write(
                    (len(additional_data_mime))
                        .to_bytes(32, byteorder='little', signed=False))
                add_size += 32/8
                hexm_file.write(additional_data_mime)
                add_size += len(additional_data_mime)

            if args.verbose:
                print ("generate version of " + args.hex_name)
            hexm_file.write(":version:")
            add_size += len(":version:")
            hexm_file.write(
                compute_git_version(args.hex_name, verbose)
                    .to_bytes(256, byteorder='little', signed=False))
            add_size += 256/8

            if block_chain_path is not None:
                if args.verbose:
                    print ("add blockchain path")
                hexm_file.write(":bcpath:")
                add_size += len(":bcpath:")
                hexm_file.write(
                    (len(block_chain_path))
                        .to_bytes(32, byteorder='little', signed=False))
                add_size += 32/8
                hexm_file.write(block_chain_path)
                add_size += len(block_chain_path)
            ...
            if licence is not None:
                ...
                hexm_file.write(
                    (len(licence))
                        .to_bytes(32, byteorder='little', signed=False))
                add_size += 32/8
                hexm_file.write(licence)
                add_size += len(licence)
            ...
            if static_code_analysis_file is not None:
                ...
                hexm_file.write(
                    (len(static_code_analysis_mime))
                        .to_bytes(32, byteorder='little', signed=False))
                add_size += 32/8
                hexm_file.write(static_code_analysis_mime)
            ...
            add_size += 32/8
            hexm_file.write(
               (add_size).to_bytes(32, byteorder='little', signed=False))

        if args.verbose:
            print ("close file and check")
        os.close(hexm_file);
        if os.path.getsize(hexm_file) != add_size + total_size:
            print ("[error] internal size computation is not correct: "
                    + "file size = " + str(os.path.getsize(hexm_file))
